---
title: Institutional Capacity Assessment Report - `r params$department`
subtitle: Under the `r params$ministry`
...
# Generate lowest capacities (bottom 2, regardless of mean score)
lowest_capacities <- category_stats %>%
  arrange(Mean_Score) %>%
  slice_head(n = 2) %>%
  select(Category, Mean_Score)
# Generate rating counts for each category
rating_counts <- dept_data %>%
  select(all_of(capacity_categories)) %>%
...
# Precompute descriptions for strongest capacities
strongest_desc_1 <- switch(strongest$Category[1],
  ...
  "Individual job design" = "Positions exist for most key roles; Most key positions are well defined and have job descriptions; Some cases of unclear responsibilities, delegations or accountabilities; Some roles dont have job descriptions; Some problems are evident in relation to the alignment between role and salary",
  ...
  "Change management and project implementation" = "Change is approached passively and avoided until urgent; Staff do not embrace change, but they tend not resisted it either; Managers spend time driving the change, often through senior staff who they hold accountable; Implementation can be slow with lots of time spent selling ideas to staff, but can be successful",
  ...
  "Stakeholder partnerships (e.g. NGO, CSOs, FBOs)" = "Some partnerships and alliances in place; Not very active and/or troubled by some poor relationships",
  ...
  "Service as a shared value" = "The organisation recognises the importance of service but has some difficulty in following though, perhaps because of clunky, inefficient processes; Service delivery is seen as important by staff, but clients do not always feel satisfied by the interaction",
  ...
  "Managing workplace and out-of-work responsibilities" = "Physical infrastructure can be made to work well enough to suit the organisations immediate needs; Little if any room for growth; Obvious improvements could be made to help increase effectiveness and efficiency (e.g. improved maintenance; rearrangement of office space)",
  "Physical infrastructure" = "Basic access to needed technology through shared facilities (phone, fax, computer, print etc); May be moderately reliable and user-friendly",
...
strongest_desc_2 <- switch(strongest$Category[2],
  "Mission and Purpose" = "Clear expression of organisation’s reason for existence which reflects its purpose and values; held by many within the organisation and referred to often",
  "Strategic plan (Corporate or business plan)" = "The plan is in place, is up to date and links well to the mission; Most strategies in the plan have been developed into clear projects with budgets and activities well defined to guide work and program planning; Most staff are aware of the plan, use it and were consulted when it was developed; Plans are used to guide management decisions",
  "Clarity of Vision" = "Mixed understanding of what the organisation aspires to achieve; lacks clarity or specificity; held by only a few people or just something that is written on a plan but with little ownership",
  "Policy and Legal framework" = "Policy or legal framework exists but is scant, unclear and subject to many different interpretations; Critical legislation is out of date and needs review",
...
# Precompute descriptions for lowest capacities
lowest_desc_1 <- switch(lowest_capacities$Category[1],
  "Mission and Purpose" = "Clear expression of organisation’s reason for existence which reflects its purpose and values; held by many within the organisation and referred to often",
  "Strategic plan (Corporate or business plan)" = "The plan is in place, is up to date and links well to the mission; Most strategies in the plan have been developed into clear projects with budgets and activities well defined to guide work and program planning; Most staff are aware of the plan, use it and were consulted when it was developed; Plans are used to guide management decisions",
  "Clarity of Vision" = "Mixed understanding of what the organisation aspires to achieve; lacks clarity or specificity; held by only a few people or just something that is written on a plan but with little ownership",
  "Policy and Legal framework" = "Policy or legal framework exists but is scant, unclear and subject to many different interpretations; Critical legislation is out of date and needs review",
  ...
  "Physical infrastructure" = "Physical infrastructure can be made to work well enough to suit the organisations immediate needs; Little if any room for growth; Obvious improvements could be made to help increase effectiveness and efficiency (e.g. improved maintenance; rearrangement of office space)",
  "Technological infrastructure" = "Basic access to needed technology through shared facilities (phone, fax, computer, print etc); May be moderately reliable and user-friendly",
  "Unknown description"
)

lowest_desc_2 <- switch(lowest_capacities$Category[2],
  "Mission and Purpose" = "Clear expression of organisation’s reason for existence which reflects its purpose and values; held by many within the organisation and referred to often",
  "Strategic plan (Corporate or business plan)" = "The plan is in place, is up to date and links well to the mission; Most strategies in the plan have been developed into clear projects with budgets and activities well defined to guide work and program planning; Most staff are aware of the plan, use it and were consulted when it was developed; Plans are used to guide management decisions",
  "Clarity of Vision" = "Mixed understanding of what the organisation aspires to achieve; lacks clarity or specificity; held by only a few people or just something that is written on a plan but with little ownership",
  "Policy and Legal framework" = "Policy or legal framework exists but is scant, unclear and subject to many different interpretations; Critical legislation is out of date and needs review",
...
# Generate top 10 priorities
top_10_priorities <- category_stats %>%
  filter(Mean_Score <= 2.5) %>%
  arrange(Mean_Score) %>%
  slice_head(n = 10)
recommendations <- c(
  "Develop a clear vision statement through staff workshops",
  "Review and update policy/legal framework with legal experts",
  "Implement training on environmental responsiveness",
  "Establish an inclusive strategic planning committee",
  "Introduce regular plan review schedules",
  "Enhance budget training and align with GoV processes",
  "Integrate programs with mission through a review process",
  "Redesign structure with updated roles and chart",
  "Revise job descriptions and align with salary scales",
  "Update and enforce operational procedures"
)
n_rows <- nrow(top_10_priorities)
if (n_rows > 0) {
  top_10_priorities <- top_10_priorities %>%
    mutate(Recommended_Action = recommendations[seq_len(n_rows)])
} else {
  top_10_priorities <- top_10_priorities %>%
    mutate(Recommended_Action = NA_character_)
}

# Find the category with the lowest mean score for the urgent section
lowest <- category_stats %>%
  arrange(Mean_Score) %>%
  slice_head(n = 1)
...
abbreviations <- tibble(
  Abbreviation = c("CSO", "FBO", "HRM", "ICA", "ICAG", "IT", "M&E", "NGO", "NSDP", "OECD", "OPSC", "PSC", "UNDP"),
  Definition = c("Civil Society Organization", "Faith-Based Organization", "Human Resource Management", "Institutional Capacity Assessment", "Institutional Capacity Assessment Grid", "Information Technology", "Monitoring and Evaluation", "Non-Governmental Organization", "National Sustainable Development Plan", "Organisation for Economic Co-operation and Development", "Office of the Public Service Commission", "Public Service Commission", "United Nations Development Programme")
)
kable(abbreviations, 
      booktabs = TRUE, 
...
We would like to express our sincere gratitude to the staff of the `r department` for their participation in the Institutional Capacity Assessment (ICA) survey. Their valuable insights and contributions have been instrumental in identifying key areas for capacity development. We also thank the Vanuatu Public Service Commission (PSC) for initiating and supporting this assessment process. Special thanks to the Vanuatu Bureau of Statistics for digitizing the `r year` PSC ICA report, which has facilitated the automation of data processing, analysis, and report generation for upcoming years' ICA surveys. This collaborative effort underscores our commitment to strengthening public institutions in alignment with the National Sustainable Development Plan (NSDP).

# INTRODUCTION

The NSDP’s society pillar goal 6 provides for “strong and effective institutions for ensuring a dynamic public sector with good governance principles and strong institutions delivering the support and services expected by all citizens of Vanuatu”. OPSC’s overall development program and policy objectives for the work of individual or institutional capacity assessment and development is also guided by this goal.

This report provides some background to the ICA process undertaken in August 2025 with the `r department` staff and provides the results or outcome of the assessment. The `r department` is encouraged to review and verify the report, and make appropriate plans to address any shortfall or improvements in institutional capacity areas or elements identified through the assessment.
...
```{r}
#| label: fig-Capacity-people-context-environment
#| fig-cap: "Capacity – people, context & environment"
#| fig-pos: "H"
#| fig-cap-location: bottom
#| out-width: "300px"
knitr::include_graphics("img/Capacity-people-context-environment.png")
```

It is important to understand the idea of “capacity” as an emergent property and is “living and dynamic” (Fowler & Ubels, 2010, p. 17) rather than as a fixed and easily measurable “thing”. Capacity can manifest in many ways: from an individual’s choice to exercise a key skill to achieve a task result, to a workgroup’s ability to coordinate actions to process a complex request, through to the coordinated delivery of public services to the community.

```{r}
#| label: fig-factors-influencing-capacity
#| fig-cap: "Key factors influencing ‘capacity’"
#| fig-pos: "H"
#| fig-cap-location: bottom
#| out-width: "400px"
knitr::include_graphics("img/factors-influencing-capacity.png")
```

@fig-factors-influencing-capacity provides important ideas to guide the way we can examine and understand capacity and how capacity might be developed in the line agencies of the public service. It seeks to bring these ideas together, by identifying the key factors that are likely to influence capacity and the development of capacity in this context and environment:
...
## How `r department`’s institutional capacity was assessed
...
This assessment examined a range of institutional capacity elements within the `r department`. A total of `r nrow(dept_data)` `r department` staff completed the institutional capacity assessment in August 2025.
...
```{r}
#| label: fig-five-steps-assessing-planning-capacity-development
#| fig-cap: "Five-Step Process for Assessing and Planning Capacity Development Using the ICAG"
#| fig-pos: "H"
#| fig-cap-location: bottom
#| out-width: "500px"
...
The Institutional Capacity Assessment Grid (ICAG) was used to gather information about the organizational capacity of different departments within the Vanuatu Public Sector, including the `r department`. This tool helps evaluate key areas like organizational design, human resource management, operational systems, leadership practices, and infrastructure. The survey was conducted using Microsoft Office 365 Forms. They rated 31 capacity categories on a scale from 1 (Clear Need) to 4 (High) based on statements that described how well the department was performing (e.g., from "little shared understanding" to "clear and compelling vision").

The survey responses were collected through Microsoft Office 365 Forms and saved into a spreadsheet. This file included answers from various departments, such as the `r department` and the Department of Water Resources. Each row in the spreadsheet represents one respondent, with details like their ID, email, name, province, ministry, department, and their ratings for the 31 capacity categories, written as descriptive text based on their chosen score.
...
1. **Data Extraction**: The survey responses were changed into a format that could be easily analyzed.
2. **Data Cleaning**: The text answers (e.g., "little shared understanding") were turned into numbers based on the ICAG scale:
    - A = 1 (Clear Need)
    - B = 2 (Basic)
    - C = 3 (Moderate)
...
The prepared data was analyzed using R to understand the `r department`’s institutional capacity. The process included the following steps:

1. **Loading the Dataset**: The survey data was brought into R[^1], and only the responses from the `r department` were selected.
2. **Calculating Mean Scores**: We figured out the average scores for each of the 31 capacity categories based on the `r department`’s responses.
3. **Identifying Strongest and Weakest Capacities**: The top two categories with the highest average scores were noted as the strongest, and categories with averages of 2 or lower were marked as the weakest.
4. **Rating Distribution**: We counted how many times each score (1 to 4) appeared for each capacity category.
5. **Visualization Data**: The average scores were prepared for charts, with one chart showing categories from most to least developed and another from least to most developed.
6. **Visualization**: Charts were created using R. The first shows average scores ordered from most to least developed, and the second ranks them from least to most developed, using colors to highlight the scores and lines to show different levels of need.
...
The Institutional Capacity Assessment Grid (ICAG) scale, ranging from 1 to 4, was carefully designed to reflect the varying levels of organizational capacity within the Vanuatu Public Sector, including the `r department`. This scale—where 1 represents "Clear Need," 2 indicates "Basic," 3 denotes "Moderate," and 4 signifies "High"—was developed based on qualitative descriptors tailored to the local context, drawing from established organizational development frameworks. The descriptors for each level (e.g., "little shared understanding" for 1, "clear and compelling vision" for 4) were crafted to capture the spectrum of performance across 31 capacity categories, ensuring relevance to the department’s operational and strategic challenges.

The analysis methods, including the calculation of mean scores and the establishment of thresholds for identifying weakest capacities, were selected to provide a robust, evidence-based foundation for actionable priorities. Mean scores were computed by averaging the ratings provided by the `r nrow(dept_data)` `r department` staff who participated in the August 2025 assessment, offering a reliable aggregate measure of capacity across each category. The threshold of ≤ 2 was chosen to flag capacities requiring urgent attention, as scores at or below this level indicate performance that is either at "Clear Need" or just reaching "Basic," signaling significant gaps that hinder effective service delivery and strategic goal attainment. This cutoff was determined to align with the National Sustainability Development Plan’s (NSDP) goal of strengthening public institutions, ensuring that the `r department` can prioritize resource allocation and capacity-building efforts where they are most needed. This approach enables the department to focus on critical areas, such as infrastructure or responsiveness, while supporting long-term planning and improvement.

## Overview of Results

As a public institution with a broad scope and mandate for implementing the national government’s priorities, the MALFFB Corporate Plan, the Department Business Plan, and other priorities of the National Sustainability Development Plan (NSDP), the `r department` acknowledges its challenges, including limited financial and human resources. The Department has undertaken to shift its approach toward working closely with, and harnessing the available resources offered through other government line agencies, as well as credible non-government, civil, and faith-based organizations. This shift necessitates the development and consolidation of specific skills and capacities.

To build an evidence base for capacity development to support this transition, staff from the `r department` participated in an institutional capacity assessment activity conducted by the Office of the Public Service Commission (OPSC) in August 2025. The assessment, based on the Institutional Capacity Assessment Grid (ICAG), involved `r nrow(dept_data)` staff members and evaluated 31 capacity categories to identify strengths and areas for improvement.

The analysis reveals that the `r department` excels in strategic planning and mission clarity, with mean scores reflecting a moderate to high capacity in these areas. However, critical gaps persist in infrastructure, responsiveness to environmental changes, and financial management, necessitating targeted interventions to enhance overall performance. These findings provide a foundation for the Department to prioritize resource allocation and capacity-building efforts, aligning with the NSDP’s goal of strengthening public institutions.

```{r}
#| label: fig-summary-bar
#| fig-cap: "Summary of results by category (institutional average)"
#| fig-pos: "H"
#| fig-cap-location: bottom
ggplot(fig3_data, aes(x = reorder(Category, -Mean_Score), y = Mean_Score, fill = Mean_Score)) +
  geom_bar(stat = "identity", width = 0.7) +
  scale_fill_gradient2(low = "#8B0000", mid = "#FF0000", high = "#00FF00", midpoint = 2.5) +
  geom_hline(yintercept = c(1.0, 1.5, 2.5, 3.5), linetype = "dashed", color = c("#8B0000", "#FF0000", "#FFA500", "#00FF00")) +
  annotate("text", x = Inf, y = 1.0, label = "Clear need", hjust = 1.1, vjust = -0.5, color = "#8B0000", size = 3) +
  annotate("text", x = Inf, y = 1.5, label = "Basic", hjust = 1.1, vjust = -0.5, color = "#FF0000", size = 3) +
  annotate("text", x = Inf, y = 2.5, label = "Moderate", hjust = 1.1, vjust = -0.5, color = "#FFA500", size = 3) +
  annotate("text", x = Inf, y = 3.5, label = "High", hjust = 1.1, vjust = -0.5, color = "#00FF00", size = 3) +
  coord_flip() +
  labs(x = "Capacity Categories", y = "Level of Capacity (1 = Clear Need, 4 = High)") +
  theme_minimal() +
...
Based on @fig-summary-bar, capacities that indicate a moderate to high level include:

- **`r strongest$Category[1]`**, described as `r strongest_desc_1`, with a mean score of `r strongest$Mean_Score[1]`.
- **`r strongest$Category[2]`**, described as `r strongest_desc_2`, with a mean score of `r strongest$Mean_Score[2]`.
...
```{r}
#| label: tbl-summary-capacities-strongest
#| comment: ""
kable(strongest, 
      booktabs = TRUE, 
      col.names = c("Capacity Category", "Mean Score"),
      caption = "Summary of Strongest Capacities") %>%
  row_spec(0, bold = TRUE, background = "#006498", color = "white") %>%
  kable_styling(latex_options = "HOLD_position", font_size = 8) %>%
...
The OPSC analysis identifies the following 10 capacity areas as priorities for development, based on mean scores ≤ 2.5, indicating performance at or below the "Basic" level. These priorities are critical for the `r department` to address to enhance institutional effectiveness and align with NSDP goals. Recommended actions are provided to guide decision-making and resource allocation.

```{r}
#| label: tbl-top-10-priorities
top_10_priorities <- category_stats %>%
  filter(Mean_Score <= 2.5) %>%
  arrange(Mean_Score) %>%
  slice_head(n = 10)
...
kable(top_10_priorities,
      booktabs = TRUE, 
      col.names = c("Capacity Category", "Mean Score", "Recommended Action"),
      linesep = "",
      caption = "Development Priorities and Recommendations") %>%
  row_spec(0, bold = TRUE, background = "#006498", color = "white") %>%
  kable_styling(latex_options = "HOLD_position", font_size = 8) %>%
...
#| label: check-means-and-display
#| include: true
#| echo: false

# Precompute dynamic values
dept_name <- department
lowest_cat1 <- lowest_capacities$Category[1]
lowest_desc1 <- lowest_desc_1
lowest_score1 <- round(lowest_capacities$Mean_Score[1], 2)
lowest_cat2 <- lowest_capacities$Category[2]
lowest_desc2 <- lowest_desc_2
lowest_score2 <- round(lowest_capacities$Mean_Score[2], 2)
# Check if all mean scores are >= 2
all_means_above_2 <- all(category_stats$Mean_Score >= 2, na.rm = TRUE)

if (all_means_above_2) {
  # Define num_other_priorities for the if branch
  num_other_priorities <- nrow(top_10_priorities)
  # Build the Markdown text string for means >= 2
  text_output <- paste0(
    "The ", dept_name, " demonstrates commendable effort and capacity across most assessed areas, with all mean scores exceeding 2, reflecting a solid foundation in delivering its mandate. This achievement highlights the dedication of the staff and the effectiveness of current practices. However, there are still areas where improvement is needed to enhance overall performance. The following capacities, identified as the lowest scoring, warrant focused attention:\n\n",
    "- **", lowest_cat1, "**, described as ", lowest_desc1, ", with a mean score of ", lowest_score1, ".\n",
    "- **", lowest_cat2, "**, described as ", lowest_desc2, ", with a mean score of ", lowest_score2, ".\n\n",
    "While these areas show mean scores above the 'Basic' level, there is still work to be done to elevate them to at least a 'Moderate' level, supporting the ", dept_name, "’s strategic objectives.\n\n"
  )
  table_data <- lowest_capacities  # Use lowest_capacities for the table
} else {
  # Filter for capacities with mean < 2
  urgent_capacities <- lowest_capacities %>% 
    filter(Mean_Score < 2)
  
  # Define num_other_priorities dynamically based on number of urgent capacities
  num_other_priorities <- nrow(top_10_priorities) - nrow(urgent_capacities)
  
  # If there are capacities with mean < 2, build dynamic bullet points
  if (nrow(urgent_capacities) > 0) {
    # Generate descriptions dynamically using switch (assuming lowest_desc_1, lowest_desc_2 are precomputed for lowest_capacities)
    bullet_points <- sapply(seq_len(nrow(urgent_capacities)), function(i) {
      desc <- if (i == 1) lowest_desc_1 else if (i == 2) lowest_desc_2 else "Unknown description"
      paste0("- **", urgent_capacities$Category[i], "**, described as ", desc, ", with a mean score of ", round(urgent_capacities$Mean_Score[i], 2), ".\n")
    })
    bullet_points <- paste(bullet_points, collapse = "")
    
    text_output <- paste0(
      "The ", dept_name, " shows varying levels of capacity across assessed areas, with some mean scores falling below 2, indicating critical gaps that require urgent attention to strengthen its ability to deliver its mandate. While certain strengths exist, the following ", ifelse(nrow(urgent_capacities) == 1, "capacity", "capacities"), ", identified as having mean scores below the 'Basic' level, highlight areas where immediate action is essential to improve performance:\n\n",
      bullet_points, "\n",
      ifelse(nrow(urgent_capacities) == 1, "This area", "These areas"), ", alongside the other ", num_other_priorities, " priorities listed in the top 10 priorities table must be addressed promptly to elevate capacity to at least a 'Moderate' level, aligning with the ", dept_name, "’s strategic objectives.\n\n"
    )
    table_data <- urgent_capacities  # Use urgent_capacities for the table
  } else {
    # Fallback if no capacities have mean < 2
    num_other_priorities <- nrow(top_10_priorities)
    text_output <- paste0(
      "The ", dept_name, " shows varying levels of capacity, with some areas requiring attention to strengthen its ability to deliver its mandate. However, no capacities were identified with mean scores below 2. The following capacities, identified as the lowest scoring, still warrant focus to improve performance:\n\n",
      "- **", lowest_cat1, "**, described as ", lowest_desc1, ", with a mean score of ", lowest_score1, ".\n",
      "- **", lowest_cat2, "**, described as ", lowest_desc2, ", with a mean score of ", lowest_score2, ".\n\n",
      "These areas, alongside the other ", num_other_priorities, " priorities listed in the table, should be addressed to elevate capacity to at least a 'Moderate' level, aligning with the ", dept_name, "’s strategic objectives.\n\n"
    )
    table_data <- lowest_capacities  # Use lowest_capacities for the table
  }
}
...
# Output the text
knitr::asis_output(text_output)

# Output the table
kable(table_data, 
      booktabs = TRUE, 
      col.names = c("Capacity Category", "Mean Score"),
      caption = "Summary of Capacities Requiring Attention") %>%
  row_spec(0, bold = TRUE, background = "#006498", color = "white") %>%
  kable_styling(latex_options = "HOLD_position", font_size = 8) %>%
  column_spec(1, width = "7cm")
...
The following table provides descriptive statistics (mean, standard deviation, minimum, maximum, and median) for each capacity category based on the responses from `r nrow(dept_data)` staff members in the `r department`.

```{r}
#| label: tbl-desc-stats
#| tbl-cap: "Descriptive Statistics by Capacity Category"
kable(desc_stats, 
      booktabs = TRUE, 
      col.names = c("Category", "Mean", "SD", "Min", "Max", "Median"),
      caption = "Descriptive Statistics by Capacity Category") %>%
  row_spec(0, bold = TRUE, background = "#006498", color = "white") %>%
...
# Appendix B: Planning for Institutional Capacity Development

The following process is offered as a means by which `r department` can:

- Make sense of the results of the ICAG
- Identify key priorities and strategies to address capacity needs, and
- Ensure where necessary that the strategies are captured in 2026 and onwards business plans.
...
**Step 3: Decide the preferred strategies and integrate into the 2026 Business Plan and subsequent plans.**

This step is an important part of recognising the importance of the change that is needed by embedding it into the priorities for 2026 and subsequent years. Inclusion in annual business plans will also ensure that the work is budgeted for and that progress is monitored as part of the plan review.
